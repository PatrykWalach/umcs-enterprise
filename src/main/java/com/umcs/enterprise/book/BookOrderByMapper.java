...
public interface BookOrderByMapper {
    ...
    default Sort.Order bookOrderByToSort(BookOrderByDTO orderBy){
        ...
        return map.entrySet().stream().filter(e->Objects.nonNull(e.getValue())).findAny().map(entry-> new Sort.Order(entry.getValue(), entry.getKey())).orElse(null);
    };


    static record BookOrderByDTO(Sort.Direction popularity, Sort.Direction price, Sort.Direction releasedAt) {

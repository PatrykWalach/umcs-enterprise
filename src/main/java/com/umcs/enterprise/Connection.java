...
public class Connection<T> {
	...
	public Connection(
		Function<Pageable, Page<T>> repository,
		LongSupplier count,
		...
		if (
			!(
				first() != null || Objects.equals(first(), 0) || last() != null || Objects.equals(last(), 0)
			)
		) {
	...
	private void edgesToReturn() {
		...
		this.edges =
			IntStream
				.range(0, page.getNumberOfElements())
				.mapToObj(i -> new Edge<>(page.getContent().get(i), (this.offset + i)))
	...
	public Optional<String> endCursor() {
		this.edgesToReturn();
		return edges.size() > 0
			? Optional.of((edges.get(edges.size() - 1)).getCursor())
			: Optional.empty();
	}
}

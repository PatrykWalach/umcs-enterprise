...
public class PurchaseDataFetcher {
	...
	@NonNull private  final BasketService basketService;
	private final BookPurchaseRepository bookPurchaseRepository;


	@DgsData(parentType = "Viewer")
	public Connection<Purchase> purchases(@InputArgument PurchaseWhere where, DataFetchingEnvironment dfe){
		new Pageable(){
	...
	@DgsMutation
	@Secured("USER")
	public MakePurchaseResult makePurchase(@InputArgument MakePurchaseInput input) throws JsonProcessingException {
		Basket basket = basketService.getBasket(input.getBasket().getId());

		var purchase = purchaseService.save(Purchase.newBuilder().build());

		purchase.setBooks(bookPurchaseRepository.saveAll(
				 (basket.getBooks().stream().map((book)->Mappers.getMapper(BookEdgeMapper.class).bookEdgeToBookPurchase(book, purchase)).toList())
		));

		return  MakePurchaseResult.newBuilder().purchase(purchase).basket(basketService.getBasket(null)).build();

	}

	@DgsMutation
	public CompletableFuture<Purchase> sendPurchase(@InputArgument SendPurchaseInput input, DgsDataFetchingEnvironment dfe) throws JsonProcessingException {
		GlobalId<String> from = GlobalId.from(input.getID());
		assert  Objects.equals(from
				.className(), "Purchase");


		return dfe.<UUID, Purchase>getDataLoader(PurchaseDataLoader.class).load(UUID.fromString(from.databaseId()))
				.thenApply(purchase-> {

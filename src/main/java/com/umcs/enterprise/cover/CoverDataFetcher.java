...
public class CoverDataFetcher {
		...
		return dataLoader
			...
			.thenApply(cover ->
				cloudinary
					.url()
					.transformation(
						new Transformation<>().gravity(getGravity(transformation.getGravity()))
								.aspectRatio(Optional.ofNullable(transformation.getAspectRatio()).map(aspect->""+aspect.getWidth()+":"+aspect.getHeight()).orElse(null))
	...
	public  String getCrop(Crop crop){if(crop ==null){
		return  null;
	}
	...
	public String getFormat(Format format){	if(format ==null){
		return  null;
	}
	...
	public DataFetcherResult<List<com.umcs.enterprise.types.Transformation>> covers(
		...
		return DataFetcherResult
			...
			.data(widths.stream().map(width -> com.umcs.enterprise.types.Transformation.newBuilder()
					...
					.aspectRatio(finalTransformation.getAspectRatio()).build() ).collect(Collectors.toList()))

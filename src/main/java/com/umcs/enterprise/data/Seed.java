...
public class Seed {
    ...
    @Bean public Jackson2RepositoryPopulatorFactoryBean jackson2RepositoryPopulatorFactoryBean() throws Exception {
    ...
    private Covers upload(String url, String uuid)  {
        ...
        try {
            ...
            result = cloudinary.uploader().upload(url, Map.of(
                    "public_id", uuid,
                    "overwrite", false,
//                    "notification_url", "https://mysite.com/webhook_endpoint"
                    "responsive_breakpoints", Map.of(
                            "create_derived", true,
                            "bytes_step", 20_000,
                            "min_width", 100,
                            "max_width", 2000
                            ...
                            "transformation",
                            new Transformation<>()
                                    .aspectRatio("3:4").crop("fill_pad")
                                    .gravity("auto").quality("auto")
                                    .fetchFormat("auto").generate()
                    )
            ));
        } catch (IOException e) {
           return null;

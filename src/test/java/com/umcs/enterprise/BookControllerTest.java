...
class BookControllerTest {
	...
	void createBook_admin() throws JSONException {
		//        given
		var input = new CreateBookInput(new CreateBookCoverInput(""), "The Book", "The Author", 100);
		...
		this.graphQlTester.documentName("BookControllerTest_createBook")
			...
			.entity(String.class)
			.isEqualTo(input.author())
			.path("createBook.price")
			.entity(String.class)
			.isEqualTo(input.price().toString())
	...
	void createBook_user() throws JSONException {
		...
		this.graphQlTester.documentName("BookControllerTest_createBook")
			...
			//                then
			.errors()
			.expect(e -> Objects.requireNonNull(e.getMessage()).contains("Unauthorized"))
			.verify()
			.path("createBook")
			.valueIsNull();
